Sub  Main( Opt, AppKey )
	...
	RunTestPrompt  AppKey.NewWritable( GetParentFullPath( SearchParent( "vbslib Prompt.vbs" ) ) )
End Sub
...
Sub  Test_current( tests )
	If IsEmpty( tests.CurrentTest.Delegate ) Then
		...
		g("Config")  = "Release"
		g("ExtraRelease") = Array( _
			"..",_
			"..\..\..\..\..\..\_replica\scriptlib\vbslib\CheckEnglishOnly",_
			"..\..\..\..\..\..\..\scriptlib\vbslib\CheckEnglishOnly" )
		'==============================================================================
...
Sub  Test_check( tests )
	...
	is_updated = False
	Set paths = ArrayFromWildcard2( "Released" )
	For Each  step_path  In  paths.FilePaths
		If not IsSameBinaryFile( step_path,  "Released\"+ step_path,  Empty ) Then
			is_updated = True
			Exit For
		End If
	Next
	...
	If is_updated Then
		For Each  path  In  g("ExtraRelease")
			Assert  g_fs.FolderExists( path )

			copy  "Release\CheckEnglishOnly.exe",  path
		Next

		For Each  step_path  In  paths.FilePaths

			copy_ren  step_path,  "Released\"+ step_path
		Next
	End If
	Pass
End Sub
